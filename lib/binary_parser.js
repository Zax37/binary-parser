...
Parser.prototype.sizeOf = function() {
  ...
  if (Object.keys(PRIMITIVE_TYPES).indexOf(this.type) >= 0) {
    ...
    // if this is a fixed length bit-level parser
  } else if (this.type === "Bit") {
    size = this.options.length / 8;
  } else if (!this.type) {
    size = 0;
  }

  if (this.next) {
    if (this.type === "Bit" && this.next.type !== "Bit") {
      size = Math.ceil(size);
    }
  ...
  if (isHash) {
    ...
    if (this.options.alreadyExists) {
      if (type instanceof Parser) {
        ctx.pushCode("{0}[{2}] = Object.assign({0}[{2}], {1});", lhs, item, counter);
      } else {
        ctx.pushCode("{0}[{2}] = {1};", lhs, item, counter);
      }
      ctx.popScope();
    } else {
...
Parser.prototype.generateNest = function(ctx) {
  ...
  if (this.options.type instanceof Parser) {
    if (this.varName && !this.options.alreadyExists) {
